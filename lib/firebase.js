...
class FsQuery {
  ...
  // .not('campo', 'is', null) → filtra client-side onde campo != null
  not(f, op, v)  { this._notF = this._notF || []; this._notF.push({ field: f, op, value: v }); return this; }
  gte(f, v)      { this._gteF = this._gteF || []; this._gteF.push({ field: f, value: v }); return this; }
  lte(f, v)      { this._lteF = this._lteF || []; this._lteF.push({ field: f, value: v }); return this; }
  gt(f, v)       { this._gtF  = this._gtF  || []; this._gtF.push({ field: f, value: v }); return this; }
  lt(f, v)       { this._ltF  = this._ltF  || []; this._ltF.push({ field: f, value: v }); return this; }
  ilike(f, v)    { this._ilikeF = this._ilikeF || []; this._ilikeF.push({ field: f, value: v }); return this; }
  ...
  async _doSelect() {
    ...
    if (this._orderF) {
      const numericColls = ['risps']; // coleções com ordenação numérica no campo 'nome'
      const isNumeric = numericColls.includes(this._coll) && this._orderF === 'nome';
      docs.sort((a, b) => {
        let va = a[this._orderF] ?? '', vb = b[this._orderF] ?? '';
        let cmp;
        if (isNumeric) {
          cmp = (parseInt(String(va).replace(/\D/g,'')) || 0) - (parseInt(String(vb).replace(/\D/g,'')) || 0);
        } else {
    ...
    if (this._notF?.length) {
      docs = docs.filter(d => this._notF.every(({ field, op, value }) => {
        if (op === 'is' && value === null) return d[field] != null;
        if (op === 'is') return d[field] !== value;
        return true;
      }));
    }
    if (this._gteF?.length) docs = docs.filter(d => this._gteF.every(f => d[f.field] >= f.value));
    if (this._lteF?.length) docs = docs.filter(d => this._lteF.every(f => d[f.field] <= f.value));
    if (this._gtF?.length)  docs = docs.filter(d => this._gtF.every(f  => d[f.field] >  f.value));
    if (this._ltF?.length)  docs = docs.filter(d => this._ltF.every(f  => d[f.field] <  f.value));
    if (this._ilikeF?.length) {
      docs = docs.filter(d => this._ilikeF.every(({ field, value }) => {
        const pat = value.replace(/%/g, '');
        return String(d[field] ?? '').toLowerCase().includes(pat.toLowerCase());
      }));
    }

    // Limit após ordenação e filtros client-side
